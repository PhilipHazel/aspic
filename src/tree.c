...
/* Functions for maintaining a binary balanced tree. I try to sneak these
functions into pretty well every program of any size that I write. */
...
/* This set of functions maintains a balanced binary tree using the algorithm
given in Knuth Vol 3 page 455.

The functions make use of uschar * pointers as byte pointers, so as to be able
to do arithmetic on them, since ANSI Standard C does not permit additions and
subtractions on void pointers. */
...
/* The node->name field must (obviously) be set, but the other fields need not
be initialized.
